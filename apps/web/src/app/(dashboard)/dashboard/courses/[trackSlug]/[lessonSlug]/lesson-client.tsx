  ...
  blocks: [
    ...
    },
    {
      ...
      data: {
        personaName: "Mr. Miyagi",
        commentary:
          'Pay close attention to the word "inspired." Neural networks are a mathematical abstraction. Real neurons are far more complex. When someone claims AI "thinks like a brain," demand their evidence.',
      },
    },
    {
      id: "b3",
...
function LessonBlock({ block }: { block: (typeof lesson.blocks)[number] }) {
  switch (block.type) {
    case "text":
  ...
  return (
    <div className="rounded-xl border border-ridgemont-warmGold-500/20 bg-ridgemont-warmGold-500/5 p-5">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm mb-3">
        <div>
          <span className="text-[10px] uppercase tracking-wider text-ridgemont-cassetteGray-500">Today</span>
        ...
        <div>
          ...
          <p className="font-medium text-ridgemont-warmGold-200 mt-0.5">{data.eraEquivalent}</p>
        </div>
      </div>
      <p className="text-sm text-ridgemont-cassetteGray-300 leading-relaxed">{data.explanation}</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-ridgemont-cassetteGray-700 bg-ridgemont-offBlack-900 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 border-b border-ridgemont-cassetteGray-700">
        <span className="text-xs text-ridgemont-cassetteGray-500 font-mono">{data.language}</span>
      </div>
      <pre className="p-4 overflow-x-auto text-sm leading-relaxed">
        <code className="text-ridgemont-arcadeGreen-300 font-mono">{data.code}</code>
      </pre>
      {data.caption && (
        <div className="border-t border-ridgemont-cassetteGray-700 px-4 py-2">
          <p className="text-xs text-ridgemont-cassetteGray-500 italic">{data.caption}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="rounded-xl border border-ridgemont-cassetteGray-700 bg-ridgemont-offBlack-800/60 p-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-ridgemont-cassetteGray-400 mt-0.5">{data.sourceName}</p>
          <p className="text-sm text-ridgemont-cassetteGray-300 mt-2 leading-relaxed">{data.excerpt}</p>
          {data.sourceUrl && (
            <a
              href={data.sourceUrl}
              target="_blank"
    ...
    <div className="rounded-xl border border-ridgemont-neonMagenta-500/20 bg-ridgemont-neonMagenta-500/5 p-5">
      ...
      {!submitted ? (
        ...
        <div className="flex items-start gap-2">
          ...
          <p className="text-sm text-ridgemont-cassetteGray-300">{data.explanation}</p>
        </div>
      )}
    </div>
  );
...
function LabBlock({ data }: { data: any }) {
  return (
    <div className="rounded-xl border border-ridgemont-arcadeGreen-500/20 bg-ridgemont-arcadeGreen-500/5 p-5">
      ...
      <p className="text-sm text-ridgemont-cassetteGray-300 mt-1 leading-relaxed">{data.description}</p>
      <Link
        href={data.href}
